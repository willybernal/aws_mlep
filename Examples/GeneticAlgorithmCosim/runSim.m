...
%% bcvtbdir = '/home/ubuntu/mlep/bcvtb'
...
rFolder = '/home/ubuntu/cosim/';
ep.removeFolderOnAws(rFolder, false);

%% Push to EC2 instance and run simulation
lFolder = 'mlep.zip';
disp('Pushing files to AWS');
rFolder = '/home/ubuntu/';
cmd = ['scp -o StrictHostKeyChecking=no -i ' ep.keyPath ' ' lFolder ' ubuntu@' ep.ec2Info.pubDNSName ':' rFolder ' &'];
[stastus, msg] = system(cmd, '-echo');
cmd = ['unzip /home/ubuntu/mlep.zip'];
feed = true;
mlepSendCommand(ep.ec2Info.pubDNSName, cmd, ep.keyPath, feed);
% Run executable
cmd = '.super';
mlepSendCommand(ep.ec2Info.pubDNSName, cmd, ep.keyPath, feed);
